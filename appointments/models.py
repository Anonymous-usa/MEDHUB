...
class AppointmentRequest(models.Model):
    ...
    class Meta:
        ...
        ordering = ('-created_at',)
        # Один активный запрос от одного пациента к одному врачу
        unique_together = ('patient', 'doctor')

    def clean(self):
        # Пациент не может назначить сам себя или юзер не своей роли
        if self.patient_id == self.doctor_id:
            raise ValidationError(_('Пациент и врач не могут быть одним и тем же пользователем'))
        if self.patient.user_type != self.patient.UserType.PATIENT:
            raise ValidationError(_('Создавать заявку может только пациент'))
        if self.doctor.user_type != self.doctor.UserType.DOCTOR:
